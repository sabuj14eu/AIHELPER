...
### Fixed — a required variable for an optional service blocked every command

1.9.0 added SearXNG with `${SEARXNG_SECRET:?...}`. On the box that produced:

```
error while interpolating services.searxng.environment.SEARXNG_SECRET:
required variable SEARXNG_SECRET is missing a value
```

on **`docker compose up`, on `docker compose exec`, on everything** — for a
service that is off by default (`WEB_SEARCH_ENABLED=false`). The owner could
not run an unrelated `exec ai-helper` command until it was set.

**Existing deployments, run this once:**

```
cd /home/shyam/ai-helper && echo "SEARXNG_SECRET=$(openssl rand -hex 32)" >> .env
```

**The cause was not the `:?`.** Compose cannot scope a requirement to a
service — profiles do **not** defer interpolation, verified rather than
assumed: a profiled service's `:?` still blocks every command. So a
hard-required variable is required of *every* deployment, and the only honest
way to ship one is to ship the thing that creates it. `AUTH_SECRET`,
`N8N_ENCRYPTION_KEY`, `WEBUI_SECRET_KEY` and `POSTGRES_PASSWORD` are all
generated by `scripts/setup.sh`; `SEARXNG_SECRET` was not, and that was the
whole defect.

- `scripts/setup.sh` now generates it with the others.
- The `:?` message says how to fix it (`openssl rand -hex 32`) rather than
  only what is missing. The person reading it is at a shell, mid-deploy, with
  everything blocked.
- **A test now closes the class, not the instance:** every `${VAR:?}` in
  `docker-compose.yml` must appear in `.env.example` *and* be generated by
  `scripts/setup.sh`. Reverting `setup.sh` to its shipped state makes it fail
  with the exact sentence above, which is how it was checked.
