...
## 2026-09-15 — P1 queue deployed; Contabo checkout converged on main
Every P1 code finding of the isolation audit is on a box and witnessed
(brother-developer `docs/audits/P1_CLOSURE_2026-09-15.md`, box column):
- ISO-06 on the Windows bridge via `patch_v7_bridge_iso06_box.py` (54f3acd,
  the box variant; the generic script aborted on the A1-patched import line,
  nothing written). Backup `.bak.20260915_171729`; `/health` 52834417,
  global_stop CLEAR, seen store dir writable.
- ISO-07 + ISO-08 on Contabo via git, not a patch script: the only runtime
  file differing from 66bc480 was `sniper_executor.py` (the c1618f5 bridge
  copy, never run here — `sniper-bot.service` runs `gunicorn bot:app`; copy
  kept as `sniper_executor.py.contabo.bak.<stamp>`); `bot.py core risk
  filters` diff 0 lines; `git reset --hard 66bc480`, `git pull --ff-only
  origin main` → 54f3acd; restart; active; HARD STOP 0; `no_account_id` 0.
THE RULE ABOVE ("Box deploy stays via the idempotent patch scripts") IS
RETIRED FOR CONTABO: from here a v7 bot deploy is `git pull --ff-only origin
main` + `sudo systemctl restart sniper-bot` + the log/`/health` witness. The
Windows bridge (`C:\Users\Administrator\sniper_executor.py`) is still a
patched copy and still deploys by patch script until it is converged too.
Still open (decisions, not code): ISO-19 shadow soak n≥20; v18 resting
pending-order expiry policy; weekly AI budget; `closed_at` clock;
USA500 in BIAS_CORE_SYMBOLS; `USE_DEMO=false` label (P2, trade_mode is
measured); reporter V18 key file on Windows. Real money stays NO-GO.
